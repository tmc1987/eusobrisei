...
class RecommendationTests(unittest.TestCase):
    ...
    def test_upgrade_ram_not_emitted_without_structural_evidence(self):
        conn = HTTPConnection("127.0.0.1", self.port, timeout=3)
        conn.request("GET", f"/v1/recommendations/devices/{self.agent['device_id']}")
        drec = json.loads(conn.getresponse().read().decode())
        conn.close()
        types = [x["type"] for x in drec["items"]]
        self.assertNotIn("upgrade_ram", types)
