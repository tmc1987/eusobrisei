...
class ActionExecutorTests(unittest.TestCase):
    ...
    def test_cooldown_tries_alternative_action_automatically(self):
        cfg = {
            "actions": {"cooldown_seconds": 999, "allowed_actions": ["throttle_top_cpu_process", "thermal_protect"]},
            "processes": {"critical_names": [], "restart_allowlist": [], "restart_commands": {}},
        }
        ex = ActionExecutor(cfg)
        req = ActionRequest(action="throttle_top_cpu_process", reason="cpu alta", params={"target_name": "chrome.exe"}, severity="high")
        with patch.object(ActionExecutor, "_throttle_top_cpu_process", return_value=actions.ActionResult("throttle_top_cpu_process", True, "ok", outcome="mitigated")), patch.object(
            ActionExecutor, "_thermal_protect", return_value=actions.ActionResult("thermal_protect", True, "energia balanceada", outcome="mitigated")
        ):
            first = ex.execute([req], {"processes": []})[0]
            second = ex.execute([req], {"processes": []})[0]
        self.assertTrue(first.success)
        self.assertTrue(second.success)
        self.assertEqual(second.action, "thermal_protect")
        self.assertIn("alternativa aplicada durante cooldown", second.message.lower())
