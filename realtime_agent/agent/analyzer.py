...
class Analyzer:
    def __init__(self, thresholds: Dict):
        ...
        self._hung_emit_max_interval_seconds = int(thresholds.get("hung_emit_max_interval_seconds", 300))
        self._hung_emit_backoff_factor = float(thresholds.get("hung_emit_backoff_factor", 2.0))
        ...
        for pid in hung_pids:
            ...
            if not state:
                state = {
                    ...
                    "last_emitted": 0.0,
                    "count": 0.0,
                    "emit_interval_seconds": float(self._hung_emit_interval_seconds),
                }
                self._hung_tracker[pid] = state
            state["count"] += 1
            should_emit = (now - state["last_emitted"]) >= float(state["emit_interval_seconds"])
            if not should_emit:
                continue
            state["last_emitted"] = now
            state["emit_interval_seconds"] = min(
                float(self._hung_emit_max_interval_seconds),
                max(float(self._hung_emit_interval_seconds), float(state["emit_interval_seconds"]) * self._hung_emit_backoff_factor),
            )
            alerts.append(
                Alert(
                    "HUNG_PROCESS",
                    ...
                    f"Processo travado detectado: {name} (PID {pid})",
                    {
                        ...
                        "next_emit_in_seconds": round(float(state["emit_interval_seconds"]), 1),
                    },
                )
            )
        return alerts
