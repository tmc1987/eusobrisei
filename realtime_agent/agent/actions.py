        ...
        for req in requests:
            try:
                if req.action == "throttle_top_cpu_process":
                    results.append(self._throttle_top_cpu_process(snapshot))
                elif req.action == "thermal_protect":
                    results.append(self._thermal_protect())
                elif req.action == "restart_process":
                    results.append(self._restart_process(req.params.get("name"), req.params.get("pid")))
                else:
                    results.append(ActionResult(req.action, False, "Ação desconhecida"))
