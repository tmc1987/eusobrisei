...
class ActionExecutor:
    ...
    def execute(self, requests: List[ActionRequest], snapshot: Dict) -> List[ActionResult]:
        ...
        for req in requests:
            ...
            if cooldown > 0 and (now - last) < cooldown:
                alternative = self._try_alternative_during_cooldown(req)
                if alternative and alternative.success:
                    alternative.message = f"{alternative.message} (alternativa aplicada durante cooldown de {req.action})"
                    alternative.operational_context = {
                        **(alternative.operational_context or {}),
                        "cooldown_bypassed_with_alternative": True,
                        "source_action": req.action,
                    }
    ...
    def _try_alternative_during_cooldown(self, req: ActionRequest) -> ActionResult | None:
        if req.action != "throttle_top_cpu_process":
            return None
        restart = self._try_restart_fallback(req)
        if restart and restart.success:
            return restart
        fallback = self._try_safety_fallback(req, "cooldown ativo")
